...
void GPU_Utils_wakeup_wrapper(ullong num_wakeup, int my_world_rank)
{

    cudaError_t err = cudaSetDevice(my_world_rank);
...
ullong CUDA_Utils_gpus_wrapper(MBR_Array *mbr_arr_1, MBR_Array *mbr_arr_2, MBR global_mbr, ullong target_num_cells, MBR_Array *partition_results, int gpu_id, int num_gpus, int my_world_rank)
{
    //size_t free, total;
    //printf("\n");
    ...
    cudaSetDevice(my_world_rank );//gpu_id);
    ...
    // passed gpu_id as dummy
	num_candidates = CUDA_Utils_candidate_generate_wrapper(&local_mbr_arr_1, &local_mbr_arr_2, DEFAULT_MBRS_PER_BLOCK, &candidates, &global_weight, gpu_id);
...
ullong CUDA_Utils_adp_wrapper(MBR_Array *mbr_arr_1, MBR_Array *mbr_arr_2, MBR global_mbr, ullong target_num_cells, MBR_Array *partition_results)
{
    ...
    // passed 0 as dummy value
	num_candidates = CUDA_Utils_candidate_generate_wrapper(mbr_arr_1, mbr_arr_2, DEFAULT_MBRS_PER_BLOCK, &candidates, &global_weight, 0);
...
ullong CUDA_Utils_candidate_generate_wrapper(MBR_Array *mbr_arr_1, MBR_Array *mbr_arr_2, ullong num_mbrs_per_cell, 
                                            Candidates *candidates, ullong *global_weight, int my_world_rank)
{
#ifdef DEBUG
    ...
    */
    ...
    GPU_Utils_wakeup_wrapper( 10, my_world_rank);
    ...
    cudaSetDevice(my_world_rank);

    // int gpuid = get_GPUid();

    // printf("Intended set Device: %d, Actaully set Device: %d \n", my_world_rank, gpuid);
...
//     if (err != cudaSuccess) {
//         printf("CUDA Kernel Error Detected Before cudaMemcpy: %s\n", cudaGetErrorString(err));
//         exit(EXIT_FAILURE);
//     }
// //Finished
    ...
    std::cout << "Number of candidates" << dev_num_candidates  ;
    std::cout << "Global weight" << dev_global_weight  ;

    checkCudaErrors(cudaFree(cells_1));
    checkCudaErrors(cudaFree(cells_2));

    ullong num_candidates;	
    printf("device num candidates: %llu device global weight: %llu\n", dev_num_candidates, dev_global_weight);
